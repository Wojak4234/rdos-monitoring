...
def precyzyjne_szukanie_zmiennej(ds, oczekiwana):
    ...
    raise ValueError(f"Brak pożądanej zmiennej. Dostępne: {dostepne}")
...
def renderuj_modul_zasolenia():
    st.header("🌊 Monitorowanie Hydrofizyczne (Modele Przestrzenne CMEMS)")
    ...
    if not siatka_gradientu:
        st.warning("Pobieranie przerwane. Serwer Copernicus mógł odrzucić zapytanie - odczekaj chwilę.")
        return
